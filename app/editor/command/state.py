...
from app.editor.controllers.command_interface import CommandControllerInterface
from app.editor.models.command import Mode
...
class BaseState(ABC):
    ...
    def post_handle(self,
                    cmd: Command | CmdTree,
                    need_reset: bool) -> tuple[CmdTree | None, bool]:
        if not cmd:
...
class NormalState(BaseState):
    ...
    def handle_key(self,
                   ...
                   prev_cmd: CmdTree | None,
                   cmd_buf: str,
                   key: Key | str) -> tuple[CmdTree | None, bool]:
        ...
        command, need_reset = self._cmd_factory.build_command(prev_cmd, cmd_buf, key)
...
class InsertState(BaseState):
    ...
    def handle_key(self, command_controller: CommandControllerInterface, prev_cmd: CmdTree | None, cmd_buf: str,
                   key: Key | str):
        if not super().pre_handle(command_controller, key):
            return None, True
        if key == "\n":
